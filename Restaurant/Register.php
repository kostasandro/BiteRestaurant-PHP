        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($nameErr == "" && $SurnameErr == "" && $emailErr == "" && $passwordErr == "" && $cpasswordErr == "" && $genderErr == "") {
                ...
                if ($result = $conn->query("SELECT * FROM USERS WHERE EMAIL= '$_POST[email]'")) {
                    ...
                    if ($count == 0) {
                        $sql = "INSERT INTO customer (First_name,Last_name,Gender)
                                VALUES ('$_POST[name]', '$_POST[surname]', '$_POST[gender]')";
                        
                        if (mysqli_query($conn, $sql)) {
                            $customer_id = mysqli_insert_id($conn);
                            
                            $sql2 = "INSERT INTO users (Role,Email,Password,Employee_ID,Customer_ID)
                                VALUES ('01', '$_POST[email]', '$_POST[password]', null," . $customer_id . ")";
            ...
            <form method="post"  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" >
                ...
                <div class="form-group row">
                    ...
                    <div class="col-8">
                        ...
                        <input type="radio" name="gender" <?php if (isset($gender) && $gender == "male") ; ?>  id="gender" value="male"> Male                 
                    </div>
                </div>
